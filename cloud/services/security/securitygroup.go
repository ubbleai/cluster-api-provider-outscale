...
func (s *Service) CreateSecurityGroupRule(securityGroupId string, flow string, ipProtocol string, ipRanges string, securityGroupMemberId string, fromPortRange int32, toPortRange int32) (*osc.SecurityGroup, error) {
	var rules []osc.SecurityGroupRule
	if securityGroupMemberId != "" && ipRanges == "" {
		...
		return nil, errors.New("Get Both ipRanges and securityGroupMemberId")
	} else {
		// One rule needs to be created for every ipRanges in the list,
		...
		for _, ipRange := range strings.Split(ipRanges, ",") {
			rules = append(
				rules, osc.SecurityGroupRule{
					IpProtocol:    &ipProtocol,
...
func (s *Service) DeleteSecurityGroupRule(securityGroupId string, flow string, ipProtocol string, ipRanges string, securityGroupMemberId string, fromPortRange int32, toPortRange int32) error {
	var rules []osc.SecurityGroupRule
	if securityGroupMemberId != "" && ipRanges == "" {
		securityGroupMember := osc.SecurityGroupsMember{
			SecurityGroupId: &securityGroupMemberId,
